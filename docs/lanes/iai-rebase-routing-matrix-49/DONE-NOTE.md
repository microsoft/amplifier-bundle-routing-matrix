...
| 4 | PR marked ready with a comment naming what was reconciled — never merged | **DONE** — `draft=false`, comment posted, never merged. Went ready → draft → ready on owner direction; see "PR review state" below |
...
```
ready_for_review  bkrabach  2026-09-03T02:25:42Z   <- lane, reading the item literally
convert_to_draft  bkrabach  2026-09-03T02:31:50Z   <- owner directed draft
ready_for_review  bkrabach  2026-09-03T02:33:29Z   <- owner directed ready again (final)
```

The item's literal text is *"mark the PR ready with a comment naming exactly
what was reconciled"* (task line), and *"PR marked ready … (**or** a DRAFT
superseding PR naming #49 **if the head branch is unreachable**)"* (deliverable
line) — DRAFT appears only in a fallback conditioned on the head branch being
unreachable, which it was not. The lane marked it ready on that reading, was
directed to draft, was then directed back to ready, and the final state agrees
with the item text.

**Why all three transitions are left visible.** Deleting the middle one would
make the timeline read as if the question never arose. It did arise, twice, and
the next person to touch #49 is better served by seeing that than by a clean
line. Nothing about the code changed across any of it: same head tree, same 570
passing tests, `merged=false` throughout, `origin/main` never moved off
`d1d7128`.
